...
def track_user_car(surr_id: int, car_type: str, lang: str, stop_event=None) -> None:
    ...
    if notification_type == "number-in-queue":
        ...
        while True:
            ...
            car_time_in_queue = car_time_in_queue_message(current_status[3], lang) if current_status else None
            ...
            if status_code == 3:  # summoned
                send_telegram_message(car_type_icon + " " + CARTRACKING[lang]["car_summoned"].format(car) + "\n" + car_time_in_queue, user_id)
                try:
                    deactivate_user_car_notification_in_db(surr_id)
                except Exception as e:
                    print(f"[WARN] Failed to deactivate {surr_id}: {e}")
    ...
    # ---------- "every-n-cars" ----------
        ...
        while True:
            ...
            if status_code == 2 and position < notification_value:
                send_telegram_message(car_type_icon + " " + CARTRACKING[lang]["car_moved_forward"].format(car, position, current_status[0].capitalize())
                                       + "\n" + car_time_in_queue, user_id)
                try:
                    deactivate_user_car_notification_in_db(surr_id)
                except Exception as e:
